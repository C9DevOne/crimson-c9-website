...
- **`shadcn` contradicts [`ADR-0007`](./adr/0007-radix-tailwind-component-layer.md), which is Accepted.** The ADR records building directly on Radix primitives and explicitly setting shadcn aside. The codebase has `shadcn` as a dependency, a `components.json`, and generated components in `src/components/ui/`. One of the two has to give: either remove the drift, or write an ADR superseding 0007. Not a decision to make silently in a review comment — it's exactly the "why didn't we just use X" case ADRs exist for. Lean is to keep shadcn and write a superseding ADR — the generated components are genuinely built on real Radix underneath, and the token duplication this raised has been fixed (below) — but the ADR itself hasn't been written yet. _(Note: `components.json` also points at `src/app/globals.css`, but the file lives at `src/app/(frontend)/globals.css` — stale either way.)_
- ~~shadcn's semantic tokens (`--primary`, `--secondary`, `--destructive`, etc.) duplicated brand-token hex values instead of referencing them~~ **Resolved.** `globals.css` now aliases every shadcn token to the brand token it matched (`--card: var(--background)`, `--ring: var(--brand-crimson)`, etc.) instead of repeating the hex. `--destructive` stays a standalone value on purpose — it's not a brand colour. `--sidebar-primary-foreground` was the one non-matching value (`#ffffff` vs. `--foreground`'s `#f5f5f5`); aliased anyway since the sidebar itself is slated for removal.
...
- **Duplicate dependencies to collapse.** Both `framer-motion` and `motion` (its renamed successor) are installed; both `lenis` and the deprecated `@studio-freight/lenis`; and `ogl` is present but documented nowhere and may be unused. Each pair should become one. Related to [`ADR-0008`](./adr/0008-animation-library-framer-vs-gsap.md) but not blocked by it — the duplication is wrong regardless of which library wins.
